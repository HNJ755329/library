...
public:
    ...
    T dfs(int i, int par)
    {
        ...
        {
            auto &e = G[i][l[i]];
            dpl[i][l[i] + 1] = M::merge(dpl[i][l[i]], M::apply(dfs(e.to, e.rev), e.val));
            ++l[i];
        }
        while (r[i] != par && r[i] >= 0)
        {
            auto &e = G[i][r[i]];
            dpr[i][r[i]] = M::merge(dpr[i][r[i] + 1], M::apply(dfs(e.to, e.rev), e.val));
            --r[i];
        }
        if (par < 0)
            return dpr[i].front();
        return M::merge(dpl[i][par], dpr[i][par + 1]);
    }
...
//     // 正確にはノードaとノードbに親子関係はない．左からあつめたaと右からあつめたbの合体．
//     static T merge(T a, T b) { return {a.first*b.first, a.second+b.second}; }
//     // 子どものノードを親のノードに足す前処理的な。
//     static T apply(T a, U b) { return {a.first, a.second+b}; }
