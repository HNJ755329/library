...
struct Trie
{
    ...
    {                                           // 頂点を
        ...
        std::vector<int> accept;                // 末端がこの頂点になる文字列の str_id を保存
        int c;                                  // base からの間隔をint型で表現したもの
        int common;                             // いくつの文字列がこの頂点を共有しているか
        std::vector<long long int> common_char; //文字cを含む子孫の数
    ...
    void insert(const std::string &word) { insert(word, nodes[0].common); }
